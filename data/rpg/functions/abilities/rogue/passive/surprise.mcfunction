...
execute positioned ^ ^ ^1 as @e[dx=1,tag=!this] run tag @s add target
...
execute as @e[tag=targeted] run scoreboard players operation @s rpg.status.stunned = @a[tag=this,limit=1] rpg.level.class_level
execute as @e[tag=targeted] run scoreboard players add @s rpg.status.stunned 3
